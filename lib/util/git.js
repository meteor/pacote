...
function fullClone (repo, committish, target, opts) {
  opts = optCheck(opts)
  const gitArgs = ['clone', '--mirror', '-q', repo, '.git']
...
function updateSubmodules (localRepo, opts) {
  return statAsync(
    path.join(localRepo, '.gitmodules')
  ).then(
    stat => stat.isFile(),
    // The error identifier has to appear in the body of the handler to
    // silence the handle-callback-err linting rule, hence false && error:
...
function execGit (gitArgs, gitOpts, opts) {
  opts = optCheck(opts)
  return BB.resolve(
    gitOpts && gitOpts.cwd && mkdirpAsync(gitOpts.cwd)
  ).then(() => cwdOwner(gitOpts, opts)).then(() => checkGit(opts)).then(gitPath => {
